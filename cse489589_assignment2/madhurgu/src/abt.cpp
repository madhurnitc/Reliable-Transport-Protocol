...
pkt packet;
int A_seqnumber = 0;
int B_expectedSequenceNumber = 0;
bool waitingForACK;
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
 	...
 	A_application++;
    // if there are no packets waiting for ACK , transmit this message.
	if (!waitingForACK)
	{

		//calculate the sequence number
		A_seqnumber = A_seqnumber % 2;
		//set the value for seqnum in the packet
		packet.seqnum = A_seqnumber;
		//set the ack value in the packet
		packet.acknum = 0;

		//calculate the checksum and set  pay-load in packet
		for (i = 0; i < 20; i++) {
		...
		// set the checksum value in packet
		packet.checksum = checksum;
		tolayer3(0, packet);
		//increase A_transport
	    A_transport++;
		//set the waiting for ACK to true.
		waitingForACK = true;
...
void A_input(struct pkt packet)
{
    ...
    if ((checksum == packet.checksum) && (packet.acknum == A_seqnumber))
    {
		//increase the A's sequence number
		A_seqnumber++;
    	waitingForACK = false;
    	stoptimer(0);
    }
...
void B_input(struct pkt packet)
{
    ...
    checksum = checksum + packet.acknum + packet.seqnum;
    //check if the above calculated checksum is same as what is stored in the packet
    // if it is same we extract the data and send the ACK to A otherwise a negative ACK
    if ((checksum == packet.checksum) && (packet.seqnum == B_expectedSequenceNumber))
    {
		tolayer5(1, &packet.payload[0]);
		//increase B_application
		B_application++;
		ackPacket.acknum = packet.seqnum;
		ackPacket.seqnum = packet.seqnum;
		for (i = 0; i < 20; i++) {
			ackPacket.payload[i] = '1';
			ackchecksum = ackchecksum + ackPacket.payload[i];
		}
		//add the sequence number and ack number to it
		ackchecksum = ackchecksum + ackPacket.acknum + ackPacket.seqnum;
		ackPacket.checksum = ackchecksum;
		tolayer3(1, ackPacket);
		//increase B_expectedSequenceNumber
		B_expectedSequenceNumber++;
		B_expectedSequenceNumber = B_expectedSequenceNumber%2;
     }
     else if ((checksum != packet.checksum) || (packet.seqnum != B_expectedSequenceNumber))
     {
		ackPacket.acknum = (B_expectedSequenceNumber+1)%2;
		ackPacket.seqnum = 0;
		for (i = 0; i < 20; i++)
		{
			ackPacket.payload[i] = '1';
