...
int gbnBase;
int gbnNextSeqNumber;
int refusedPacketCount =0;
int refusedPacketSeqNumber = 0;
int refusedPacketSent = 0;
pkt gbnSenderBuffer[1000];
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
	...
	checksum = 0;
	int packetwindow = gbnBase + WINSIZE;
	printf("GBNBASE = %d GBNNEXTSEQUNCENUMBER = %d GBNBASE+WINDOW = %d \n",gbnBase,gbnNextSeqNumber,packetwindow);

	// check if next sequence number falls in the current window
	if ( gbnNextSeqNumber < gbnBase+WINSIZE)
	{
		printf("Sending the packet with Sequence Number = %d \n", gbnNextSeqNumber);
		// set the sequence number for the packet
		gbnSenderBuffer[gbnNextSeqNumber].seqnum = gbnNextSeqNumber;
		//set the ack value in the packet
		gbnSenderBuffer[gbnNextSeqNumber].acknum = 0;
		// calculate the checksum (data+ack+seq number) as well set the payload for packet and save it in sender buffer
		for (i=0 ; i< 20; i++)
		{
			checksum = checksum + message.data[i];
			gbnSenderBuffer[gbnNextSeqNumber].payload[i] = message.data[i];

		}
		//add the sequence number and ack number to the checksum calculated above
		checksum = checksum + gbnSenderBuffer[gbnNextSeqNumber].acknum + gbnSenderBuffer[gbnNextSeqNumber].seqnum;
		//set the checksum in packet
		gbnSenderBuffer[gbnNextSeqNumber].checksum = checksum;

		// send the data to layer 3
		tolayer3(0,gbnSenderBuffer[gbnNextSeqNumber]);
		//increase A_transport
		A_transport++;

		//check if gbn Base and next Sequence number are same,
		//if yes , then start the timer
		if ( gbnBase == gbnNextSeqNumber )
		{
	...
	else
	{
        ...
        printf("Buffering the packet\n");

		//set the sequence number for the packet in refused packet buffer and global gbn buffer
		refusedPacketBuffer[refusedPacketCount].seqnum = gbnNextSeqNumber;
		gbnSenderBuffer[gbnNextSeqNumber].seqnum = gbnNextSeqNumber;
		// set the ack number for the packet in refused packet buffer and global gbn buffer
		refusedPacketBuffer[refusedPacketCount].acknum = 0;
		gbnSenderBuffer[gbnNextSeqNumber].acknum = 0;
		// calculate the checksum (data+ack+seq number) as well as set the payload for packet and save it in the packet and in sender buffer
		for (i=0 ; i< 20; i++)
		{
			checksum = checksum + message.data[i];
		...
		//add the sequence number and ack number to the checksum calculated above
		checksum = checksum + gbnSenderBuffer[gbnNextSeqNumber].acknum + gbnSenderBuffer[gbnNextSeqNumber].seqnum;
		//set the checksum in refused packet buffer and global gbn buffer
...
void A_input(struct pkt packet)
{
    ...
    {
    	//increment the base
    	gbnBase = packet.acknum + 1;
    	//if base and next sequence number are same , stop the timer
    	if( gbnBase == gbnNextSeqNumber)
    	{
    		stoptimer(0);
    	}
        ...
        printf("Acknowledgment received for the packet with Sequence Number = %d \n",packet.acknum);
        int packetwindow = gbnBase + WINSIZE;
    	printf("A_Input : gbnbase = %d gbnBase+win = %d \n",gbnBase,packetwindow);
    	printf("Total Refused Packets = %d Refusedsent = %d \n",refusedPacketCount,refusedPacketSent);
    	...
    	{
    		if((refusedPacketBuffer[i].seqnum > gbnBase) && (refusedPacketBuffer[i].seqnum < packetwindow) )
    		{
...
{
	int i;
    printf("A Timer interrupt called\n");
	//restart the timer
	starttimer(0,TIMEOUT);
	// as this is gbn ,resend all the packets in the window [gbnBase ...to...gbnNextSeqNumber-1]
	for (i=gbnBase ; i<gbnNextSeqNumber ; i++)
	{
		// send the packet to layer 3
		printf("Resending the packet with Sequence Number = %d \n",gbnSenderBuffer[i].seqnum);
		tolayer3(0,gbnSenderBuffer[i]);
		//increase A_transport
		A_transport++;
...
{
	//initialize gbn base and sequence number
	gbnBase = 1;
	gbnNextSeqNumber = 1;
...
void B_input(struct pkt packet)
{
    ...
    checksum = checksum + packet.acknum + packet.seqnum;
    printf("Calculated checksum = %d  Packet Checksum = %d \n", checksum,packet.checksum);
    //check if the above calculated checksum is same as what is stored in the packet and
    //sequence number is equal to expected sequence number
    // we extract the data and send the ACK to A
    if ( ( checksum == packet.checksum ) && ( packet.seqnum == gbnExpectedSeqNumber) )
    {
    	printf("Received Packet %d with out any corruption\n",packet.seqnum);
    	//send the data to upper layer
    	tolayer5(1,packet.payload);
    	//increase B_application
    	B_application++;
    	//set ack and seq number for the ACK packet to A
    	gbnAckPacket.seqnum = 0;
    	gbnAckPacket.acknum = gbnExpectedSeqNumber;
    	// fill the pay-load with 1s
    	//calculate the value for pay-load
    	for ( i=0 ; i<20 ;i++)
    	{
    		gbnAckPacket.payload[i] = '1';
    		ackchecksum = ackchecksum + gbnAckPacket.payload[i];
    	}
    	//add the acknum and sequence number to ackchecksum
    	ackchecksum = ackchecksum + gbnAckPacket.acknum + gbnAckPacket.seqnum;
    	// set this checksum in ACK packet
    	gbnAckPacket.checksum = ackchecksum;
    	printf("Acknowledgment sent for the packet with seq number = %d \n", packet.seqnum);
    	//send the ACK packet to A
    	tolayer3(1,gbnAckPacket);
    	//increase expected sequence number
    	gbnExpectedSeqNumber++;

    }
    else
    {
    	//send the previous ACK packet to A
    	tolayer3(1,gbnAckPacket);
    }
}
